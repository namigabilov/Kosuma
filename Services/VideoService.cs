...
namespace Kosuma.Services
{
    public static class VideoService
    {
        ...
        public static void AddEndlistTag(string m3u8FilePath)
        {
            if (File.Exists(m3u8FilePath))
            {
                var lines = File.ReadAllLines(m3u8FilePath).ToList();
                if (!lines.Contains("#EXT-X-ENDLIST"))
                {
                    lines.Add("#EXT-X-ENDLIST");
                    File.WriteAllLines(m3u8FilePath, lines);
                }
            }
        }
        ...
        public static void ConvertSegmentToTs(string streamId, string inputFile, string outputDirectory)
        {
            ...
            string segmentFilePath = $"{outputDirectory}/data%010d.ts";
